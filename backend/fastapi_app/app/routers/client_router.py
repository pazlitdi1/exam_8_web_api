...
from fastapi_app.app.schemas import ClientRegisterSchema, ClientLoginSchema, PasswordResetSchema
from fastapi_app.app.database import Session, ENGINE
from fastapi_app.app.models import Client, Admin
...
async def get_client(Authorization: AuthJWT = Depends()):
    try:
        ...
        check_admin = session.query(Admin).filter(Admin.username == Authorization.get_jwt_subject()).first()
        if check_admin:
