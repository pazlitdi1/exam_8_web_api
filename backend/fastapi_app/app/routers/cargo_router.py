...
from fastapi_app.app.schemas import CargoCreateSchema, CargoUpdateSchema
...
async def create_cargo(request: CargoCreateSchema, Authorization: AuthJWT = Depends()):
    try:
...
async def delete_cargo(id: str, Authorization: AuthJWT = Depends()):
    try:
        Authorization.jwt_required()
    except Exception as e:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail='Invalid Token')
