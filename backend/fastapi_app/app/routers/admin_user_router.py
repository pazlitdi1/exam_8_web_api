...
from fastapi_app.app.schemas import UserRegisterSchema, UserLoginSchema, PasswordResetSchema
from fastapi_app.app.database import Session, ENGINE
from fastapi_app.app.models import Admin, Client
...
async def get_users(query: Optional[str] = Path(..., min_length=1), authorization: AuthJWT = Depends()):
    try:
        ...
        if query:
            users = session.query(Admin).filter(
                (Admin.username.ilike(f"%{query}%")) | (Admin.email.ilike(f"%{query}%"))
            ).all()
        else:
...
async def login_user_admin(request: UserLoginSchema, authorization: AuthJWT = Depends()):
    check_user = session.query(Admin).filter(Admin.username == request.username).first()
...
async def user_register(request: UserRegisterSchema):
    check_user = session.query(Admin).filter(
        or_(
...
async def change_password(username: str, user: PasswordResetSchema, Authorization: AuthJWT = Depends()):
    try:
        Authorization.jwt_required()
        check_user = session.query(Admin).filter(Admin.username == username).first()
        if not check_user:
